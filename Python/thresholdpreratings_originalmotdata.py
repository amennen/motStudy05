...
for s in np.arange(len(subtouse)):
    ...
    for st in np.arange(nstim):
        ...
        for w in np.arange(nwin):
            ...
            # this counts the number of time points together
...
FILTERED_TRmatrix_consec = TRmatrix_consec
...
for s in np.arange(len(subtouse)):
    ...
    FILTERED_TRmatrix_consec[~stimkeep, :, s] = np.nan
...
allr2 = getcorr(FILTERED_RTsim,FILTERED_TRmatrix_consec, 'RTsim', 'TRmatrix')
...
#plt.fill_between(catrange[:-1], y2-ye2, y2+ye2,facecolor=allpallet[3],alpha=0.3)
#plt.plot(catrange[:-1],y2, color=allpallet[4], linewidth=6)
...
## now plot
bins = np.arange(-1.5,1.6,.05)
x = plt.hist(x2,bins, normed=True)
nx = x[0]/np.sum(x[0])
plt.plot(catrange,nx, 'blue')
