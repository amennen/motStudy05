...
% validate or not
function [patterns trials stimOrder ] = OLDGetSessionInfoRT(subjNum,SESSION,behav_dir,varargin)
%subjNum = 1;
...
r = dir(fullfile(behav_dir, ['mot_realtime05_' num2str(subjNum) '_' num2str(SESSION)  '*.mat']));
load(fullfile(behav_dir,r(end).name));
...
nTrials = length(stim.cond);
TH = find(stim.cond==1); %for recall this is fast dot motion
TE = find(stim.cond==2); %for recall this is slow dot motion
LH = find(stim.cond==3); %for recall this is omit trials
LE = find(stim.cond==4); %for recall there's no condition 4
trials.hard = TH;
trials.easy = TE;
trials.lure = [LH LE];
[~,stimOrder.hard] = sort(stim.id(TH));
[~,stimOrder.easy] = sort(stim.id(TE));
[~,stimOrder.lure] = sort(stim.id(LH));
