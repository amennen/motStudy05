...
for s = 1:length(svec)
    subj = svec(s);
    behavioral_dir = [fileparts(which('mot_realtime05.m')) '/BehavioralData/' num2str(subj) '/'];
    fileSpeed = dir(fullfile(behavioral_dir, ['mot_realtime05_' num2str(subj) '_' num2str(MOT_PREP)  '*.mat']));
    matlabOpenFile = [behavioral_dir '/' fileSpeed(end).name];
    lastRun = load(matlabOpenFile);
    allSpeed(s) = MAX_SPEED - lastRun.stim.tGuess(end);
end

behavioral_dir = [fileparts(which('mot_realtime05.m')) '/BehavioralData/' num2str(thisSubj) '/'];
fileSpeed = dir(fullfile(behavioral_dir, ['mot_realtime05_' num2str(thisSubj) '_' num2str(MOT_PREP)  '*.mat']));
matlabOpenFile = [behavioral_dir '/' fileSpeed(end).name];
lastRun = load(matlabOpenFile);
